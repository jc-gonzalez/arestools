...
def main():
    ...
    num_pids = to_pid - from_pid
    block_pids = int(num_pids / 2)

    # Get parameter names for the range of parameter ids, and retrieve samples as DataFrame
    param_names1 = data_provider.get_parameter_names_from_pids(from_pid, from_pid + block_pids)
    df1 = data_provider.get_parameter_data_df(param_names1, timestamp_start, timestamp_end)

    param_names2 = data_provider.get_parameter_names_from_pids(from_pid + block_pids + 1, to_pid)
    df2 = data_provider.get_parameter_data_df(param_names2, timestamp_start, timestamp_end)
