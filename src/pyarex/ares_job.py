...
from pyarex.parameter_sample_provider import ParameterSampleProvider

"""
# old imports, saved here until the next stable release
import pyarrow
import pyarex.data_source as ds
import pyarex.hbase_connect as hc
import pyarex.protobuf as pb
import pyarex.sample as sa
from pyarex.byte_buffer import *
from pyarex.data_provisioning_factory import *
"""
...
class AresJob:
    ...
    """
    def __init__(self, name='pyarex_job'):
        self.__name = name
        self.__app_id = None
    ...
    def execute_job(self, param_names, start, end,
        ...
        with open(data_path+'pyarex.pkl', 'wb') as file:
        ...
        cmd = ('PYSPARK_PYTHON=./SP/bin/python3 '
               ...
               '--master yarn '
               '--deploy-mode cluster '
               '--files %s#pyarex.pkl,%s#pyarex_conf.ini '
               '--archives %s#SP '
               ...
               '--executor-memory %s '
               '--num-executors %d '
               '%s' % (data_path+'pyarex.pkl', config_path+'pyarex_conf.ini',
    ...
    def get_job_logs(self, job_id=None):
        ...
        cmd_out = subprocess.check_output(cmd, shell=True, stderr=subprocess.STDOUT)
        cmd_out = cmd_out.decode("utf-8")
        output = "\n".join(re.findall(r".*PyArEx:.*", cmd_out))
        return output
    ...
    def __get_samples(self, param_name, start, end):
        ...
        dp = ParameterSampleProvider('pyarex_conf.ini')

        try:
            #samples = dp.get_parameter_data_objs([param_name], start, end)
